...
const SkillItem = ({ skill, categoryIndex, skillIndex, scrollYProgress, isMobile }: { skill: string; categoryIndex: number; skillIndex: number; scrollYProgress: MotionValue<number>; isMobile: boolean }) => {
  ...
  const textColor = isMobile ? "rgb(17 24 39)" : useTransform(
    scrollYProgress,
    ...
    >
      <div className="flex flex-col md:flex-row md:flex-wrap gap-y-12 md:gap-x-12 md:gap-y-10">
        {Object.entries(skills).map(([category, items], categoryIndex) => {
          const isLastCategory = categoryIndex === Object.keys(skills).length - 1;
          return (
            <div key={category} className={`w-full md:w-auto md:min-w-[200px] text-left ${isLastCategory ? 'border-b md:border-b-0 border-gray-200 pb-6 md:pb-0' : ''}`}>
